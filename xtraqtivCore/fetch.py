...
def list_notebooks(client=None):
    ...
    if not client:
        ...
        if not consumer_key or not consumer_secret:
            print(
                "ERROR: [list_notebooks] EVERNOTE_CONSUMER_KEY or EVERNOTE_CONSUMER_SECRET not set."
            )
            return None

        client = create_evernote_client(
            consumer_key, consumer_secret, access_token, sandbox
        )
...
def list_notes(notebook_guid=None, client=None):
    ...
    if not client:
        ...
        if not consumer_key or not consumer_secret:
            print(
                "ERROR: [list_notes] EVERNOTE_CONSUMER_KEY or EVERNOTE_CONSUMER_SECRET not set."
            )
            return []
