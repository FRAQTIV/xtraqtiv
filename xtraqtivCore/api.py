...
if not CONSUMER_KEY or not CONSUMER_SECRET:
    print(
        "ERROR: EVERNOTE_CONSUMER_KEY and EVERNOTE_CONSUMER_SECRET must be set in environment variables."
    )
...
async def start_auth():
    ...
    try:
        ...
        keyring.set_password(
            "evernote_extractor", "request_token_secret", request_token_secret
        )
...
async def auth_callback(oauth_token: str, oauth_verifier: str):
    ...
    try:
        ...
        request_token_secret = keyring.get_password(
            "evernote_extractor", "request_token_secret"
        )
        print(
            f"[KEYRING_DEBUG] /auth/callback: Retrieved request_token: {'SET' if request_token else 'NOT_SET'}"
        )
        ...
        print(
            f"[KEYRING_DEBUG] /auth/callback: Got access_token: {'SET' if access_token else 'NOT_SET'}"
        )
        print(
            f"[KEYRING_DEBUG] /auth/callback: Got access_token_secret_from_evernote: type={type(access_token_secret_from_evernote)}, value_is_truthy={bool(access_token_secret_from_evernote)}"
        )
        ...
        retrieved_access_token_after_set = keyring.get_password(
            "evernote_extractor", "access_token"
        )
        print(
            f"[KEYRING_DEBUG] /auth/callback: Verified access_token after setting: {'SET' if retrieved_access_token_after_set else 'NOT_SET'}"
        )
...
async def auth_status():
    ...
    try:
        ...
        print(
            f"[KEYRING_DEBUG] /auth/status: Retrieved access_token: {'SET' if access_token else 'NOT_SET'}"
        )
    ...
    try:
        ...
        for key in ["access_token", "request_token", "request_token_secret"]:
            try:
                ...
                print(
                    f"[KEYRING_DEBUG] /logout: '{key}' not found in keyring, or error deleting."
                )
...
async def get_user_notebooks():
    ...
    try:
        ...
        if not access_token:
            ...
            raise HTTPException(
                status_code=401, detail="User not authenticated. Please login first."
            )

        print(
            "[API_LOG] /notebooks: User authenticated, attempting to fetch notebooks."
        )
        ...
        if notebooks_data is None:  # list_notebooks might return None on error
            print(
                "[API_LOG] /notebooks: Failed to fetch notebooks (list_notebooks returned None)."
            )
            raise HTTPException(
                status_code=500, detail="Failed to fetch notebooks from Evernote."
            )

        print(
            f"[API_LOG] /notebooks: Successfully fetched {len(notebooks_data)} notebooks."
        )
