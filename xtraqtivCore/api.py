...
try:
    logger.info("[KEYRING_DEBUG] Active keyring backend: %s", keyring.get_keyring().name)
except Exception as e:
    logger.error("[KEYRING_DEBUG] Error getting keyring backend: %s", e)
...
if not CONSUMER_KEY or not CONSUMER_SECRET:
    logger.error("ERROR: EVERNOTE_CONSUMER_KEY and EVERNOTE_CONSUMER_SECRET must be set in environment variables.")
...
async def auth_callback(oauth_token: str, oauth_verifier: str):
    ...
    try:
        logger.info("[KEYRING_DEBUG] /auth/callback: Entered")
        ...
        logger.info("[KEYRING_DEBUG] /auth/callback: Retrieved request_token: %s", 'SET' if request_token else 'NOT_SET')
        
        if not request_token or not request_token_secret:
            logger.info("[KEYRING_DEBUG] /auth/callback: Request token or secret not found")
        ...
        logger.info("[KEYRING_DEBUG] /auth/callback: Got access_token: %s", 'SET' if access_token else 'NOT_SET')
        logger.info("[KEYRING_DEBUG] /auth/callback: Got access_token_secret_from_evernote: type=%s, value_is_truthy=%s", type(access_token_secret_from_evernote), bool(access_token_secret_from_evernote))

        # Store access token securely
        logger.info("[KEYRING_DEBUG] /auth/callback: Attempting to store access_token...")
        ...
        logger.info("[KEYRING_DEBUG] /auth/callback: access_token supposedly stored.")
        
        # Verify immediately if it was stored
        retrieved_access_token_after_set = keyring.get_password("evernote_extractor", "access_token")
        logger.info("[KEYRING_DEBUG] /auth/callback: Verified access_token after setting: %s", 'SET' if retrieved_access_token_after_set else 'NOT_SET')
        ...
        logger.info("[KEYRING_DEBUG] /auth/callback: Request tokens cleaned up.")
        
        return {"status": "authenticated", "message": "Authentication successful"}
    except Exception as e:
        logger.error("[KEYRING_DEBUG] /auth/callback: Exception: %s", str(e))
        raise HTTPException(status_code=500, detail=str(e))
...
async def auth_status():
    ...
    try:
        logger.info("[KEYRING_DEBUG] /auth/status: Entered")
        access_token = keyring.get_password("evernote_extractor", "access_token")

        logger.info("[KEYRING_DEBUG] /auth/status: Retrieved access_token: %s", 'SET' if access_token else 'NOT_SET')
        ...
        logger.error("[KEYRING_DEBUG] /auth/status: Exception: %s", str(e))
...
async def logout():
    ...
    try:
        logger.info("[KEYRING_DEBUG] /logout: Entered")
...
async def get_user_notebooks():
    ...
    try:
        ...
        if notebooks_data is None: # list_notebooks might return None on error
            logger.error("[API_LOG] /notebooks: Failed to fetch notebooks (list_notebooks returned None).")
            raise HTTPException(status_code=500, detail="Failed to fetch notebooks from Evernote.")

        logger.info("[API_LOG] /notebooks: Successfully fetched %s notebooks.", len(notebooks_data))
        return notebooks_data
    except HTTPException as http_exc: # Re-raise HTTPExceptions directly
        raise http_exc
    except Exception as e:
        logger.error("[API_LOG] /notebooks: Unexpected error: %s", str(e))
